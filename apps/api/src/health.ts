...
export const healthRouter = new Elysia()
  .get('/health', async () => {
    ...
    return {
      ...
      uptime,
      services: {
        database: dbCheck.status === 'fulfilled' ? { status: 'ok', latency: Date.now() - start } : { status: 'error' },
        redis: redisCheck.status === 'fulfilled' ? { status: 'ok', latency: Date.now() - start } : { status: 'error' },
        queue: queueCount.status === 'fulfilled' ? { status: 'ok', size: queueCount.value } : { status: 'error' }
      }
    }
  })
