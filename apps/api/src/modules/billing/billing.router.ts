...
import { webhookQueue, pdfQueue } from '@api/queue'
...
export const billingRouter = new Elysia({ prefix: '/billing' })
  ...
  .post(
    '/checkout',
    async ({ request, body, tenant, billingService }) => {
  ...
  .post(
    ...
    async ({ request, body, tenant, user }) => {
      ...
      let rate = 1.0
      if (body.currencyId) {
        const curr = await db.query.currencies.findFirst({
          where: eq(currencies.id, body.currencyId),
        })
        if (curr) {
      ...
      if (ipCountry) {
        const tr = await db.query.taxRates.findFirst({
          where: and(
            eq(taxRates.tenantId, tenant.id),
            eq(taxRates.countryCode, ipCountry.toUpperCase()),
          ),
        })
...
export const webhookRouter = new Elysia({ prefix: '/api/v1/webhooks' }).post(
  '/stripe',
  async ({ request, set }) => {
