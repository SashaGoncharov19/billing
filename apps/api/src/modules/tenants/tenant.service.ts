...
export class TenantService {
  ...
  async updateTenant(tenantId: string, data: Partial<{ name: string; primaryColor: string; secondaryColor: string }>) {
  ...
  async removeMember(tenantId: string, memberId: string) {
    ...
    await db.delete(memberships).where(and(eq(memberships.id, memberId), eq(memberships.tenantId, tenantId)))
    return true
  }
}
