...
- Entity Seven Automated Billing
  ...
  async onSuspend(tenantId: string, subscriptionId: string): Promise<void> {
    ...
    if (!clientId) {
      console.error(
        `[Plugin:KeyHelp] Cannot suspend: No keyhelpClientId found for sub ${subscriptionId}`,
      )
      return
    }
  ...
  async onReactivate(tenantId: string, subscriptionId: string): Promise<void> {
    ...
    if (!clientId) {
      console.error(
        `[Plugin:KeyHelp] Cannot reactivate: No keyhelpClientId found for sub ${subscriptionId}`,
      )
      return
    }
